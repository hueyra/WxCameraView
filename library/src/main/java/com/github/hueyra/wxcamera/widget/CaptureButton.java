package com.github.hueyra.wxcamera.widget;
...
public class CaptureButton extends View {
    ...
    public CaptureButton(Context context, int size) {
        ...
        button_state = WxCameraConfig.BUTTON_STATE_BOTH;  //初始化按钮为可录制可拍照
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                //判断按钮状态是否为可录制状态
                if ((button_state == WxCameraConfig.BUTTON_STATE_ONLY_RECORDER || button_state == WxCameraConfig.BUTTON_STATE_BOTH))
                    postDelayed(longPressRunnable, 500);    //同时延长500启动长按后处理的逻辑Runnable
                break;
            case MotionEvent.ACTION_MOVE:
                if (captureListener != null
                        && state == STATE_RECORDING
                        && (button_state == WxCameraConfig.BUTTON_STATE_ONLY_RECORDER || button_state == WxCameraConfig.BUTTON_STATE_BOTH)) {
    ...
    private void handlerPressByState() {
        ...
        switch (state) {
            //当前是点击按下
            case STATE_PRESS:
                if (captureListener != null && (button_state == WxCameraConfig.BUTTON_STATE_ONLY_CAPTURE || button_state ==
                        WxCameraConfig.BUTTON_STATE_BOTH)) {
    ...
    private class LongPressRunnable implements Runnable {
        ...
        public void run() {
            ...
            //没有录制权限
            if (Utils.getRecordState() != WxCameraConfig.CHECK_PERMISSION_STATE_SUCCESS) {
                state = STATE_IDLE;
                if (captureListener != null) {
